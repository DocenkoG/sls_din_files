...
local termostatSwitch = zigbee.value("home3","state")
local termostatTarget,t2 = obj.get("termostat.1.target")
local delta = 0.0
local tempC = zigbee.value("home_8715","temperature")
...
if (termostatState == "OFF") then
  zigbee.set("home3","state","OFF")
  zigbee.set("home_abd4","state","OFF")
elseif (tempC > termostatTarget + delta) then
  zigbee.set("home3","state","OFF")
  zigbee.set("home_abd4","state","OFF")
  obj.set("termostat.1.state","INACTIVE")
elseif (tempC < termostatTarget - delta) then
  zigbee.set("home3","state","ON")
  zigbee.set("home_abd4","state","ON")
